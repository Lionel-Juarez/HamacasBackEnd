...
public class SombrillaController {
    ...
    public ResponseEntity<?> updateSombrillaReserva(@PathVariable("id") Long id, @RequestParam("idReserva") Long idReserva, @RequestParam("lado") String lado) {
        LOGGER.info("Intentando actualizar sombrilla ID " + id + " con reserva ID " + idReserva + " y lado " + lado);
        return sombrillaRepositorio.findById(id).map(sombrilla -> {
            ...
            sombrilla.setLado(lado);
            sombrillaRepositorio.save(sombrilla);
            LOGGER.info("Sombrilla ID " + id + " actualizada con el lado " + lado);
